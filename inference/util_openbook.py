...
def get_contexts():
    SIM_MODEL = '../input/sentencetransformers-allminilml6v2/sentence-transformers_all-MiniLM-L6-v2'
    DEVICE = 0
    MAX_LENGTH = 384
    BATCH_SIZE = 16

    WIKI_PATH = "../input/wikipedia-20230701"
    wiki_files = os.listdir(WIKI_PATH)

    trn = pd.read_csv("../input/kaggle-llm-science-exam/test.csv").drop("id", 1)

    model = SentenceTransformer(SIM_MODEL, device='cuda')
    model.max_seq_length = MAX_LENGTH
    model = model.half()

    sentence_index = read_index("../input/wikipedia-2023-07-faiss-index/wikipedia_202307.index")
    ...
    df = pd.read_parquet("../input/wikipedia-20230701/wiki_2023_index.parquet",
                         columns=['id', 'file'])
